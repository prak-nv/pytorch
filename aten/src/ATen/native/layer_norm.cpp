...
std::tuple<Tensor, Tensor, Tensor> layer_norm_jit_cpu(
    const Tensor& input,
  ...
  return layer_norm_cpu(X, gamma, beta, M, N, eps);
}

std::tuple<Tensor, Tensor, Tensor> layer_norm_backward_jit_cpu(
    const Tensor& dY,
    const Tensor& input,
    IntArrayRef normalized_shape,
    const Tensor& mean,
    ...
    return layer_norm_backward_cpu(dY, X, mean, rstd, gamma, M, N, grad_input_mask);
}
