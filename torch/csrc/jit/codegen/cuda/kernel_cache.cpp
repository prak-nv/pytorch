...
std::vector<at::Tensor> FusionExecutorCache::runFusionWithInputs(
  ...
  if (code_to_fe_lookup_.count(unique_id) == 0) {
    ...
    if (has_reduction_) {
      ...
      // This call is relatively heavy weight, consider caching
      // Figure out all outputs that depend on the reduction for scheduling
      ...
      auto reduction_params = getReductionHeuristics(
          fusion_.get(), inputs, reduction_tv, tvOutputsOfReduction);

      launch_params = reduction_params.lparams;
