...
std::set<int> getDimsToSkip(const TensorView* tv, const TensorView* ca_tv,
                                              size_t pos) {
  ...
  if (tv->isConsumerOf(ca_tv)) {
    if (BroadcastOp *bop = dynamic_cast<BroadcastOp*>(ca_tv->getOrigin())) {
      ...
      for (size_t i = 0; i < ca_tv->domain()->domain().size(); ++i) {
        ...
        if (std::all_of(ir_utils::filterByType<IterDomain>(root_vals).begin(),
                        ir_utils::filterByType<IterDomain>(root_vals).end(),
                        [&root_dims_to_skip](IterDomain* root_id) {
                          return root_dims_to_skip.find(root_id) != root_dims_to_skip.end();
                        })) {
...
int TensorView::getComputeAtRelPos(int pos) const {
  TORCH_INTERNAL_ASSERT(hasComputeAt(),
                        "Tensor does not have a computeAt tensor.");
  // Note: pos is actually an axis index.
  TORCH_INTERNAL_ASSERT(pos < (int)getThisComputeAtAxis(),
                        "Not a computeAt axis: ", pos);
  ...
  TORCH_INTERNAL_ASSERT(pos_cav < (int)compute_at_view_->nDims(),
                        "Error computing relative position in computeAt.");
  return pos_cav;
}
