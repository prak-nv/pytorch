...
struct Operation 
{
  ...
  unsigned nsubops; // number of following operations to skip if lazy evaluated (makes possible to recover tree structure)
...
struct ReusedExprMapping
{
  ...
  // create_lazy_mapping - Allocates mapping for reused subexpression and its value slot index
  static ReusedExprMapping create_lazy_mapping(const bitvector_t& reused_sexprs) // O(n)
  {
...
struct EvalState
{
  ...
  explicit EvalState(ReusedExprMapping mapping): memo_(mapping.size()), mapping_(std::move(mapping))
  {
  }

  EvalState(const EvalState&) = default;

  // Returns pointer to slot for reused subexpression value or nullptr if subexpression is not reused
...
// All operations to calculate expression value are kept in linear tree form to allow lazy evaluation
//
// On creation of expression evaluator initial stack of known (scalar values is prepared and memoized)
// During evaluation runtime (unknow) values are being queried and inserted to the stack copy
// Afterwards expression is calculated by DFS right first tree reduction using stack algorithm
...
private:
  ...
  stack_t initial_stack_; // initial stack of known operands with gaps for runtime values
  std::vector<binding_gap_t> binding_gaps_; // list of gap indices on initial stack
