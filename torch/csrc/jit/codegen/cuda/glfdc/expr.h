...
struct ExprDAG {
 ...
 public:
  ...
  uintptr_t getBinding(UnboundValue ubv) const noexcept // O(1)
  {
    assert(ubv.index_ < unbound_values_.size());
    return unbound_values_[ubv.index_];
  }

  std::size_t reuseCount() const noexcept {
    return reuse_count_;
  }

  void markSExprReuse(SExprRef ref);
};
