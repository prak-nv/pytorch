...
// Recursive binary tree DFS traversal.
...
size_t dfs_fn(
  ...
  auto& left_fn = getters.first;
  auto& right_fn = getters.second;
  ...
  if (left.has_value())
    lchild_count = dfs_fn(left.value(), list, visit_pre, visit_post, getters);

  auto right = right_fn(t);

  if (right.has_value())
    rchild_count = dfs_fn(right.value(), list, visit_pre, visit_post, getters);
...
template <
    ...
    typename FnPost_,
    typename LRGetters_>
auto make_dfs_traversal(
    OpFn_ op_fn,
    FnPre_ visit_pre,
    FnPost_ visit_post,
    LRGetters_& getters) {
  return [op_fn, visit_pre, visit_post, getters](Tree_ t) -> void {
    ...
    dfs_fn(t, list, visit_pre, visit_post, getters);

    for (auto p : list)
      c10::guts::apply(op_fn, p);
  };
}

auto make_sexpr_child_getters(const ExprDAG& dag) noexcept {
  auto left_child_f = [&d = dag](SExprRef ref) -> c10::optional<SExprRef> {
    SExpr e = d.fetch(ref);
  ...
  auto right_child_f = [&d = dag](SExprRef ref) -> c10::optional<SExprRef> {
    SExpr e = d.fetch(ref);
  ...
  if (is_value(op)) {
    ...
    if (is_unbound_value(op)) {
      ...
      binding_gaps_.emplace_back(
          initial_stack_.size() - 1, dag().getBinding(unbound));
    }
  }
  // Not a value - child operations will handle everything
}

void ExprEvaluator::calculate_subops_operands() // O(n) - n is number of
                                                // operations
{
  ...
  // NB: This does DFS on preorder binary tree node list - we're able to recover
...
void ExprEvaluator::prepare_eval() // O(n)
{
  ...
  // Normaly we'd like to evaluate them postorder (of mirrored tree ie. right
  // child first), however for lazy evaluation those need to be checked
  // preorder.

  auto op_fn = [this](SExprRef ref, std::size_t child_count) -> void {
  ...
  auto visit_pre = [this](SExprRef ref) -> void {
    const SExpr expr = dag().fetch(ref);
    // Put left hand side operands on initial stack
    prepare_eval_operand(expr.lhs_);
  };

  auto visit_post = [this](SExprRef ref) -> void {
    const SExpr expr = dag().fetch(ref);
    // Put right hand side operands on initial stack
    prepare_eval_operand(expr.rhs_);
  };

  auto getters = make_sexpr_child_getters(dag());
  auto traversal =
      make_dfs_traversal<SExprRef>(op_fn, visit_pre, visit_post, getters);
...
void ExprEvaluator::evaluate_subexpr(
    size_t op_index,
    stack_t& eval_stack,
    EvalState& es) const {
  ...
  std::size_t rhs_op = !is_value(root_expr.lhs_)
      ? lhs_op + 1 + operations_[lhs_op].nsubops
      : lhs_op;
...
ExprEvaluator::opt_scalar_t ExprEvaluator::evaluate(
    EvalState& es,
    binding_fn_t binding_fn) const {
  if (is_value(expr_.subexpr_))
...
ExprEvaluator::opt_scalar_t ExprEvaluator::evaluate_value(
    Value val,
    binding_fn_t binding_fn) const {
  if (is_scalar(val))
    return c10::get<scalar_type>(val);
  ...
  return binding_fn(dag().getBinding(unbound));
}
...
std::unique_ptr<EvalState> glfdc::createEvalStateForDAG(const ExprDAG& dag) {
  auto mapping =
      ReusedExprMapping::create_lazy_mapping(dag.reused_subexpressions_);
